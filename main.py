...
HEALTHCHECK_INTERVAL = 6 * 60 * 60  # 6 часов
...
async def check_dates():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            err_msg = f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] ⚠️ Ошибка Playwright: {e}"
            print(err_msg)
            traceback.print_exc()
            send_telegram(f"❗ Ошибка Playwright:\n{e}")
        finally:
            await browser.close()
...
async def loop():
    ...
    while True:
        try:
            ...
            err_msg = f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] ❗ Ошибка во внешнем цикле: {e}"
            print(err_msg)
            traceback.print_exc()
            send_telegram(f"❗ Ошибка в цикле:\n{e}")

        if (datetime.now() - last_healthcheck).total_seconds() > HEALTHCHECK_INTERVAL:
